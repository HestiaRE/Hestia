...
# System key registry readers (share/hestia/sys-keys.json). Loaded here because the panel's config
# emitter and the repair both sit behind main.sh.
...
source_conf() {
	while IFS='= ' read -r lhs rhs; do
		if [[ ! $lhs =~ ^\ *# && -n $lhs ]]; then
			# A config key is a plain identifier, never an array subscript: `declare -g $lhs=` would
			# evaluate a command substitution smuggled into `key[$(...)]` (GHSA-xffx-jj33-p2px).
			[[ $lhs =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]] || continue
			# PATH and BIN pass the identifier check above. Loud, because a record carrying one is an attack.
...
# writes it, so a copy would be a second home that answers silently. HOMEDIR is the opposite case
# (no registry entry, no writer, refused by source_conf) and sits with the constants.
BACKUP_DISK_LIMIT=95
BACKUP_LA_LIMIT=$(grep -c '^processor' /proc/cpuinfo)
RRD_STEP=300
HOMEDIR='/home'
BIN=$HESTIA/bin
# sbin holds what the sudo wildcard on bin/* must NOT reach. Its own anchor, or a caller keeps
# pointing at bin/ unnoticed.
SBIN=$HESTIA/sbin
...
# Panel certificate, outside the install root (h-update-hestia replaces that wholesale) and outside
# CONF_DIR (0700, while caddy, exim and proftpd read this as non-root).
HESTIA_SSL="/etc/ssl/hestia"
# Exim interpolates the SNI name into a path, so the filenames ARE the SNI names. Flat and exact,
# never stripping a "mail." prefix: mail.kunde.de would strip to another customer's domain.
...
# Selectable vhost templates; the non-selectable ones live in share/, and the webmail ones per app
# under $HESTIA/share/$WEB_SYSTEM/webmail/ (hence no MAILTPL). PHPTPL is its own anchor and not
# $WEBTPL/$WEB_BACKEND: that is a config VALUE, and renaming the directory would then have to rename
# the value everywhere it is compared.
...
# Customer web and the webmail front are separate concepts. On a mailfront box WEB_SYSTEM is empty
# (customer web absent, and every web command's is_system_enabled guard keys on that) while nginx
...
is_package_full() {
	...
	# An absent limit is a broken record, not a limit of zero, but [[ n -ge "" ]] reads it as one and
	# refuses every request blaming the customer's package. Ask the package file, where the value came from.
	case "$limit" in
		'unlimited') ;;
		'' | *[!0-9]*)
			...
			case "$limit" in
				'unlimited') return 0 ;;
				'' | *[!0-9]*)
					# Not enforcing is safe for the customer and invisible for the operator: one typo
					# in a package unlimits everyone on it. So it is said.
...
# drops SYNs costs ~45 minutes before the first error. wget -O leaves a 0-byte file on failure, which
# a plain [ -f ] accepts.
fetch_release_asset() {
...
# The wp-cli phar pinned in share/manifest.json (version + sha256). It runs as every customer, so
# never a moving or unverified source. No partial file on failure; chown stays with the caller.
fetch_wp_cli_phar() {
...
# The webmail sqlite backend needs its driver in the PANEL pool. Heals the one known ordering gap
# (package landed after hestia-php-confd built the conf.d); a missing package stays a hard failure.
...
# The default matrix (A-Za-z0-9) is sed-replacement-safe (no /, &, \) and callers rely on that when
# substituting into configs, so a custom matrix must keep the property. Draws are checked per class:
# 16 characters out of A-Za-z0-9 carry no digit in about 6 % of draws, which a database password
# policy then rejects. The filter can also return fewer characters than asked for.
generate_password() {
	local matrix="${1:-A-Za-z0-9}" length="${2:-16}" attempt=0 pass='' classes=0
	[[ "$matrix" == *A-Z* ]] && classes=$((classes + 1))
	[[ "$matrix" == *a-z* ]] && classes=$((classes + 1))
	[[ "$matrix" == *0-9* ]] && classes=$((classes + 1))
	# The one unmeetable request, knowable before the first draw, so the loop below cannot run out.
	...
	# Unreachable after the check above. Printing the last candidate beats printing nothing: an empty
	# password passed on creates an account without one.
...
# Only an absolute path is a local repository. rclone:, sftp:, rest: and s3: are not paths, so a glob
# over one stays literal and an rm -rf removes nothing while the caller believes it deleted; a
# relative path resolves against whatever directory the command started in. Anything unrecognised
# counts as remote, the safe direction to be wrong in for a destructive command.
...
parse_object_kv_list_non_eval() {
	...
	# mapfile, not `for x in $(...)`: word splitting globs, so a value holding a * comes back as a
	# filename. Captured first and the status checked, because a process substitution hides its own.
...
# split and GLOBBED before either sees a character: a record holding MIN='*' picks up any file named
# MIN='...' in the working directory, reachable wherever a customer chooses the directory a h-* runs
# in. Unquoted also collapses whitespace inside a value.
...
# Panel convention: a secret arrives as the PATH of a /tmp file so it never enters argv. Value and
# path space overlap, so a password shaped like an existing /tmp path is read as that file; safe
# because the caller is already root-equivalent via sudoers, and the symlink guard keeps the
...
# The certificate's common name, from one place. Read out of `-text` it depends on how openssl spells
# a DN, and 3.5 dropped the spaces around the `=` (deb13 and ub26 broke, deb12 and ub24 did not).
# `-nameopt multiline` prints one attribute per line. Empty means no CN, which is a real state.
cert_common_name() {
	openssl x509 -noout -nameopt multiline -subject -in "$1" 2> /dev/null | sed -n 's/^ *commonName *= //p'
}

# Escape named variables IN PLACE for a JSON string literal, by name to avoid a subshell per field
# (a 300-domain listing would fork twelve thousand times). Emitters only and only last, since it
# destroys the raw value; emit with printf %s, or the shell splits and globs the escaped value.
json_escape() {
	local _n _v _c _out
	for _n in "$@"; do
		_v="${!_n}"
		...
		_v="${_v//$'\r'/\\r}"
		_v="${_v//$'\n'/\\n}"
		# JSON forbids every raw control character, not just the three with a short form. Rare enough
		# to pay for a per-character loop only when one is present.
		if [[ "$_v" == *[$'\x01'-$'\x1f']* ]]; then
			_out=''
...
update_object_value() {
	...
	varname="${4#\$}"
	old="${!varname}"
	# The old value becomes a sed BRE pattern, so every metacharacter must be escaped or '[SPAM]'
...
# would take aXb.com's zone, and a vhost still referencing it breaks nginx -t box-wide.
remove_pool_zone() {
...
# Literal line removal for the account-keyed mail files; widening the localpart charset would
# silently under-escape a sed pattern. Owner and mode are copied onto the rewrite, or a root:root
# passwd (dovecot:mail) cuts dovecot off from auth.
...
# Same, keyed by the whole line: fwd_only holds one bare account per line, so a prefix match on
# john.doe would also take john.doex.
remove_exact_line() {
	local file="$1" line="$2"
	[ -e "$file" ] || return 0
...
update_user_value() {
	...
	if [ -n "$lnr" ]; then
		# 'c' rewrites any line, the last included, and unlike 's' has no delimiter a value could
...
send_notice() {
	...
	if [ "$notify" = 'yes' ]; then
		# Second writer of notifications.conf, so NOTICE (rendered via x-html) is sanitized here too
		# or it is an XSS bypass. %quote% keeps the record intact.
...
# drawn per write, which happens only where there is no file to preserve it from.
system_crontab_write() {
	...
	# Rename, never truncate: with fs.protected_regular=2 and the sticky, group-writable crontabs
	# directory, opening the file for writing is EACCES even for root. rename is not subject to that.
	mv -f "$_tmp" "$_dst"
}

# The periodic repair, in /etc/cron.d and NOT in the hestia crontab: a deleted crontab would take the
# line that restores it along with it. Daily at 04:40, where nothing else runs, because a week
# without a crontab is a week without queue processing. Root directly; not reachable from the panel.
system_repair_cron_write() {
	...
	# ("INSECURE MODE"), which is how the hestia-ssl fallback once ran nowhere.
...
# Names a service account owns, or will own once its component is installed. passwd and group know
# only today's box, so a curated list is the only way to keep a later component's name free.
# Callers still check passwd/group themselves.
...
# Address family by CONTENT, never by path or field, so a rename cannot change the answer. Echoes
# 4, 6, or nothing, and callers treat empty as "skip". Here rather than in ip.sh because the
# web-model consumers need it without sourcing ip.sh.
ip_family() {
	case "$1" in
		*:*) echo 6 ;;
...
# Silent predicate, exit status only. The validator below refuses AND logs, so probing with it wrote
# an [Error 2] line on every successful domain add. Named unlike it on purpose: one asks, one refuses.
looks_like_ip46() {
...
# One parser for all three families, because three copies drifted: only the v4 one got the mask
...
# Addresses and networks, both families: the common deny list rejects colon and slash, so nothing v6
# could ever be stored. An empty list is legal, since switching the feature off writes one.
is_ip_list_format_valid() {
	...
	for entry in "${entries[@]}"; do
		# OUTER whitespace only, like the panel: stripping inner spaces would store "192.168.1. 0/24",
		# which the matcher can never hit
...
# A restore selector: empty, '*', 'no'/'yes', or a comma list of object names. CLOSED character set,
# never a deny list, because the value is spliced into a queue line bash runs as root. Space is in
# (a home entry can be "my documents"), TAB is not: tar prints it escaped in the listing the restore
# matches against, so such a selector would silently select nothing.
...
is_cron_command_valid_format() {
	...
	# A crontab CMD field is a single line, so an embedded newline would inject an extra entry
	# (GHSA-5fpv). Semicolons stay legal; they are ordinary in cron commands.
	is_no_new_line_format "$1"
}
...
# A host for a URL or an ssh-style host:path: a v6 literal needs brackets, or its colons read as the
# port separator. NOT for expect (Tcl) or the ftp client; see include/backup.sh.
url_host() {
	case "$1" in
		*:*) echo "[$1]" ;;
		*) echo "$1" ;;
...
# Debian since bullseye and would select a missing binary that silently acts like nologin;
# screen, tmux, dash and rbash are out too. Existing off-list shells are kept, not offered.
HESTIA_SHELL_ALLOWLIST="nologin jailbash bash sh"

# The effective shells: allowlist as intersected with /etc/shells, one basename per line, ladder
# order. nologin is emitted unconditionally, being every new user's default shell, so it stays
# assignable even before /etc/shells is seeded.
list_allowed_shells() {
	local allow
...
# Validates by VARIABLE NAME: the name is both the type and the variable read via ${!name}, so a
# rename changes which check runs and the name is part of the interface. An UNSET variable is a hard
# error, or a typo silently disables a check; a declared-but-empty one stays a legitimate skip.
is_format_valid() {
	for arg_name in "$@"; do
...
# Always the twin of $domain as format_domain left it. Seeded unconditionally: a surviving raw
# argument like `www.other.com` makes every duplicate guard check a name no record carries, while
# record and vhost are created under the stripped name, another customer's.
format_domain_idn() {
	domain_idn=$domain
...
is_restart_format_valid() {
	# 'now' is the restart.pipe's own grammar: every h-restart-* queues itself as "$SCRIPT now" under
	# SCHEDULED_RESTART, and the queue swallows errors, so rejecting it kills every queued restart
	# silently. It means run immediately and do not requeue.
	if [ -n "$1" ]; then
...
multiphp_default_version() {
	...
	# Fall back to the newest php that actually has fpm installed.
...
# and rename: a sed expands & and \ inside the value, so "Foo & Bar" glued the old value into the
# new one and left the quotes unbalanced. A value that cannot form KEY='VALUE' is refused, not
# written. Every matching line is replaced; the duplicate collapse stays with syshealth.
change_sys_value() {
	...
	# SIGKILL never runs the trap, so a killed run leaves its temp behind. Older than five minutes
	# only, or a concurrent writer loses its own. -H because $HESTIA/conf is a symlink.
	...
	trap "rm -f '$_tmp'" EXIT
	# One subshell behind one redirect, so a full disk is rc 1 here and not a truncated file later.
	# Mode and owner from the file, never the umask: the seed sets 660.
	if (
		found=no
...
# Delete the line entirely, where change_sys_value would set it empty. The web-model switch needs it
# so a target model ends up byte-identical to a fresh install of that model.
clear_sys_value() {
...
# registry, never a bare sed: an unanchored one rewrote DB_MARIADB_SYSTEM. On a success path the
# caller fails the command with the rc; on an exit path it is dropped, so a write problem does not
# turn "not installed" into a different error.
sys_key_token_set() {
...
# From the installed package, never an argument: version from dpkg, source off the version string and
# named in the recipe's own words so recipe and status agree. Not apt-cache policy, which describes
# the repo configured NOW while the string travels with the package.
...
# The channel from the box: the upstream phar in /usr/local/bin shadows the OS package on PATH, so it
# decides when both exist. In the recipe's own words (source_default: os_package | upstream_installer),
# because that field IS the channel; same rule as DB_MARIADB_SYSTEM, opposite of PHP_SOURCE. The
# vocabulary is contract from 1d on.
...
# As the installed phar reports it, not the manifest pin: the pin is intent, the key is status.
# wp-cli refuses root without --allow-root; no phar or no answer is an empty key.
wpcli_status_record() {
	local v=''
...
# A live web-model switch holds an exclusive lock for the whole operation. Domain-config mutators
# acquire it with a bounded wait so nothing changes web/mail state mid-flip; reload chokepoints
# (h-restart-web/-proxy/-service, apache logrotate, LE renewal) defer while it is held.
WEB_MODEL_LOCK="/run/hestia/web-model.lock"

# True (0) while a switch holds the freeze, for reload chokepoints to defer. The owner and its
# subprocesses carry HESTIA_WEB_LOCK_HELD and are never frozen by their own lock.
web_freeze_held() {
...
# Reentrant (the owner and its children skip re-acquiring) and bounded, or a hung switch freezes
# every domain op and cron forever. Held via WEB_LOCK_FD until the process exits or web_lock_release
# runs; non-zero on timeout.
web_lock_acquire() {
	...
	# Who owns it, so the release can check rather than rely on what happens not to be exported.
	export HESTIA_WEB_LOCK_PID=$$
	return 0
}

# Release early; otherwise it drops on process exit. Only the acquiring process may, because a child
# inherits the lock as held and would unlock the parent mid-operation.
web_lock_release() {
...
# The sftp-jailed group is the sshd chroot selector and the pam_namespace scope; the jail itself is
# built per session.
add_chroot_jail() {
	local user=$1
...
# The one sshd "Subsystem sftp" line, on the sftp-server binary so a jailbash user's sftp runs in
# bwrap. /usr/lib/sftp-server is the compat symlink shipped on all four targets, and jailbash binds
# /usr read-only, so it resolves inside the jail too. Prints "changed" so the caller restarts;
...
jail_sshd_subsystem_apply() {
	...
	# Checked before anything is written: rebuilding a correct file moves the distro line to the
	# bottom every run and costs an sshd restart each time.
	...
	# shellcheck disable=SC2064 # expand now on purpose: _tmp is local and gone when the trap fires
	trap "rm -f '$_tmp'" EXIT
	# The line must stand in the GLOBAL section: everything after the first Match belongs to that
	# block, where sshd ignores a Subsystem and `sshd -t` stays quiet about it. So drop every such
	# line and write one before the first Match, or before the marker comment, which belongs to the
	# block it introduces.
...
# Co-maintain the SSH AllowUsers allowlist. Opt-in: acts only if a line exists, the installer seeds
# one commented. Touches only the $user token (base before @), so operator entries and the
# commented/active state survive. sshd -t rollback, reload only when active, and it re-comments
# rather than leave an active line empty.
manage_sshd_allowusers() {
	local action=$1 user=$2
	local config='/etc/ssh/sshd_config'
	[ -f "$config" ] || return 0

	# First managed AllowUsers line, commented or active; none means nothing to maintain. The '#'
	# must sit DIRECTLY on the keyword, sshd's own commented-directive form. A prose line
	# ("# AllowUsers is a login allowlist...") must not match, or the sentence gets tokenised and
	# the user appended to it while the real directive stays untouched.
	local lineno
	...
	# Swap the line on a temp copy, keep it only if sshd accepts the whole config. ENVIRON, not -v,
	# or awk reinterprets backslashes in the value.
	local tmp
	tmp=$(mktemp) || return 0
